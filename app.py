...
# --- KONEKSI AI (DENGAN AUTO-RETRY) ---
try:
    import google.generativeai as genai
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
    
    def panggil_ai_pintar(prompt):
        # Kita coba 3 kali kalau kena limit (429)
        for i in range(3):
            try:
                model = genai.GenerativeModel("gemini-1.5-flash") # Pake flash biar enteng di internet
                response = model.generate_content(prompt)
                return response.text
            except Exception as e:
                if "429" in str(e):
                    time.sleep(2) # Rehat bentar biar limitnya lewat
                    continue
                raise e
        return "Limit habis, Bro. Coba edit manual dulu ya!"
except:
...
# --- FUNGSI RAKIT ISI (RAPAT & INDENTASI AWAL PARAGRAF) ---
        ...
        if tag in paragraph.text:
            ...
            for line in lines:
                ...
                if raw_line.startswith("***"):
                    # Cuma majuin huruf pertama baris ini (Awal Paragraf)
...
with st.form("input_form"):
    ...
    with col1:
        ...
        # Input tgl murni (Tegal, {isi_ini})
        tgl_surat = st.text_input("Tanggal Surat (Contoh: 21 Februari 2026)", value="21 Februari 2026")
    with col2:
...
if submit:
    # LOADING TEXT REQUEST
    with st.spinner("Lagi dibikinin sekretaris pintar..."):
        prompt = (f"Olah instruksi: {arahan}. DILARANG buat salam/yth. "
                  "Fokus ke informasi agenda. Pisahkan bagian dengan '---'.")
        st.session_state['draf_psh'] = panggil_ai_pintar(prompt)

if 'draf_psh' in st.session_state:
    st.subheader("📝 Review & Edit Draf")
    draf_final = st.text_area("Gunakan *** untuk awal paragraf yang menjorok:", 
    ...
    if st.button("💾 Cetak & Download"):
        try:
            ...
            # 1. Update Header (Fix Tanggal Gak Double)
            h_map = {
